...
当前版本：**v2.9.24-0verf1ow**

- 基于 MHSanaei/3x-ui 项目
- 由 **0verf1ow** 维护和增强

### 最新优化 (v2.9.24)
- ✅ **流量修复**: 修复 API 动态添加用户/入站时缺失 `InboundId_Email` 前缀导致流量统计失效的 Bug
- ✅ **架构优化**: 移除 `client_traffics` 表中的 Email 唯一约束，完美支持同一邮箱在不同端口入站中的独立流量核算
- ✅ **同步增强**: 优化主从同步过程中的前缀处理，确保 API 调用与全局配置生成的 Tag 规则完全一致
